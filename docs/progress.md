...
- extracts egg height and weight with OCR
- uses the live aoe.top site data source
- computes egg candidate matches locally
- provides a GUI plus floating launcher/result windows

## Implementation Plan

1. Create project skeleton, config storage, and progress log
2. Pull and cache `https://rocom.aoe.top/data/Pets.json`
3. Port the site interval matching algorithm into Go
...
- The public data endpoint for egg matching is `https://rocom.aoe.top/data/Pets.json`.
- The site computes interval penalties in the browser from the dataset.
...
1. Site “API” is actually public JSON plus front-end interval logic, so the desktop app must replicate the matching algorithm locally.
...
- 2026-04-12: confirmed the reference repo is front-end only and contains the interval matching logic
- 2026-04-16: switched dataset source to `https://rocom.aoe.top/data/Pets.json`
- 2026-04-16: ported the aoe.top incubate interval scoring and friend portrait asset paths
